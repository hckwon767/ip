...
def process_proxy_list_to_file(url, output_filename="krlist.txt"):
    """
    주어진 URL에서 프록시 목록을 읽어와 country code가 'KR'인 줄만 필터링하고,
    지정된 형식으로 변환하여 파일에 저장합니다.

    Args:
        url (str): 프록시 목록이 있는 URL.
        output_filename (str): 결과를 저장할 파일의 이름 (기본값: 'krlist.txt').
    """
    try:
        ...
        with open(output_filename, 'w', encoding='utf-8') as outfile:
            for line in lines:
                ...
                if len(parts) == 4: # ip, port, country code, name 4가지 구성인지 확인합니다.
                    ...
                    if country_code == 'KR': # country code가 'KR'인 경우만 필터링합니다.
...
if __name__ == "__main__":
    ...
    if proxy_list_url == "YOUR_URL_HERE":
        ...
        process_proxy_list_to_file(proxy_list_url)
